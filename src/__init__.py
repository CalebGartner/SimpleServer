...
# REFERENCE: https://github.com/python/cpython/blob/3.7/Lib/http/__init__.py
# I'm afraid it's almost a one-to-one, but whatever other solution I came up with was less elegant/ideal than this . . .


@unique
class Status(IntEnum):

    def __new__(cls, _code: int, _header: str, _descriptor: str = ''):
        new_status = int.__new__(cls, _code)
        new_status._value_ = _code
        new_status.code = _code
        new_status.header = _header
        new_status.descriptor = _descriptor
        return new_status
